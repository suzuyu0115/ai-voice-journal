...
- **#9** ホーム画面を日記一覧（ジャーナルスタイル）に刷新（Supabase リアルタイム取得・連続記録ストリーク）
...
- `app/summary/[id].tsx`: **2モード対応**（表示モード／作成モード）。ヘッダーの日付テキストをタップしてカレンダーモーダル（`DatePickerModal`）で日付変更（PR #51/#52 でマージ予定）
- `app/_layout.tsx`: 起動時に匿名認証を自動初期化（PR #52 でマージ予定）
...
- `src/store/journalStore.ts`: `pendingMessages` state 管理。PR #51 で `targetDate`/`setTargetDate` 追加予定
